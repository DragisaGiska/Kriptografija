...
namespace CryptographyProject.Controller
{
    public class LoadedFilesController : IEncryptionAlgorithms
    {
        ...
        public static int numberActiveThreads;
        public static TimeSpan dateandtime = new TimeSpan(0);
        ...
        public void StartEncDec(FormModel model)
        {
            ...
            while (LoadedFilesController._THREAD_CREATOR_RUNNING==true)
            {
                ...
                Thread.Sleep(500);
                if ((queueFiles.Count == 0) && (LoadedFilesController._NUMBER_OF_THREADS==0))
                {
                    LoadedFilesController._THREAD_CREATOR_RUNNING = false;
                    dateandtime = DateTime.Now.Subtract(_TIME);

                }
            }
            loggerController.Add(" #  ENC/DEC IS FINISH #");

            string message = "#  ENC/DEC IS FINISH # PRESS THE STOP BUTTON #";
            string title = "Upozerenje!!!";
        ...
        //Nazovite ovo kao rukovatelj događajima kada se nit završi   -------
        private void ThreadEnds(FileInfo file, bool threadSuccesfull, DateTime timeStarted)
        {
            ...
            //Threads number
            if (LoadedFilesController._NUMBER_OF_THREADS > LoadedFilesController.numberActiveThreads)
            {
                LoadedFilesController.numberActiveThreads = LoadedFilesController._NUMBER_OF_THREADS;
            }
